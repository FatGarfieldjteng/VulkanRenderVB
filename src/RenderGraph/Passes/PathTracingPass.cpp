...
PathTracingPass::PathTracingPass(const Desc& desc)
    : RenderPass("PathTracing"), mDesc(desc) {}

void PathTracingPass::Setup(RenderGraph& graph, PassHandle self) {
    graph.Write(self, mDesc.colorResource, VK_IMAGE_LAYOUT_GENERAL,
                VK_PIPELINE_STAGE_2_RAY_TRACING_SHADER_BIT_KHR | VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
                VK_ACCESS_2_SHADER_WRITE_BIT | VK_ACCESS_2_SHADER_READ_BIT);
    if (mDesc.forwardPassHandle != UINT32_MAX)
        graph.DependsOn(self, mDesc.colorResource, mDesc.forwardPassHandle);
}

void PathTracingPass::Execute(VkCommandBuffer cmd) {
    ...
    if (mDesc.denoiser && mDesc.enableDenoiser) {
        ...
        // Falcor/paper: SVGF expects raw 1-SPP input (one path per pixel)
    ...
    // Composite the path-traced result to HDR color image
    if (mDesc.compositePipeline != VK_NULL_HANDLE && mDesc.compositeDescSet != VK_NULL_HANDLE) {
        ...
        if (mDesc.compareSplitX > 0) {
            ...
            pc.splitLeft      = mDesc.splitLeft;
            pc.splitRight     = (mDesc.splitRight > 0) ? mDesc.splitRight : mDesc.extent.width;
